    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(data.is_dislike==1){
                $('*[data-postid="'+post_id+'_d"]').
                    removeClass('btn-secondry').
                    addClass('btn-danger');
                
                $('*[data-postid="'+post_id+'_l"]').
                    removeClass('btn-success').
                    addClass('btn-secondry');

                    var cu_dislike=$('*[data-postid="'+post_id+'_d"]').
                    find('.dislike_count').text();
                    var new_dislike=parseInt(cu_dislike)+1;
                    $('*[data-postid="'+post_id+'_d"]').
                        find('.dislike_count').text(new_dislike);

                if(data.change_dislike==1){
                    var cu_like=$('*[data-postid="'+post_id+'_l"]').
                        find('.like_count').text();
                        var new_like=parseInt(cu_like)-1;
                        $('*[data-postid="'+post_id+'_l"]').
                            find('.like_count').text(new_like);
                        }
